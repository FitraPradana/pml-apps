...
class ScanController extends Controller
{
    ...
    public function update_scan_asset(Request $request)
    {
        ...
        if ($request->status_asset == 'DONT_EXIST') {
            DB::beginTransaction();
            try {
                ...
                //Insert Tbl Log Trans Fixed Assets
                ...
                DB::commit();
                return redirect('stock_takes')->with(['success' => 'Data Asset berhasil di Update !']);
            } catch (\Throwable $th) {
                DB::rollBack();
                throw $th;
            }
        } elseif ($request->last_status_asset == 'GOOD' and $request->status_asset == 'NEED_REPAIR') {
            DB::beginTransaction();
            try {
                // Insert Stock Take Transaction
                $trans = StockTakeTransaction::create([
        ...
        } elseif ($request->last_status_asset == 'GOOD' and $request->status_asset == 'NEED_REPLACEMENT') {
            DB::beginTransaction();
            try {
                // Insert Stock Take Transaction
                $trans = StockTakeTransaction::create([
            ...
            try {
                ...
                DB::commit();
                return redirect('log_trans_asset_view')->with(['success' => 'Data Asset berhasil di Update !']);
            } catch (\Throwable $th) {
                DB::rollBack();
                throw $th;
            }
    ...
    // public function update_scan_asset2(Request $request)
    // {
    //     //validate form
    //     $this->validate(
    //         $request,
